...
it('can add the star name and star symbol properly', async() => {
    ...
    assert.equal(await instance.symbol(), 'RUBL');
    assert.equal(await instance.name(), 'New Ruble');
});
...
it('lets 2 users exchange stars', async() => {
    ...
    let user1 = accounts[1];
    let user2 = accounts[2];
    await instance.createStar('New Star!', 7, {from: user1});
    await instance.createStar('New New Star!', 8, {from: user2});
...
it('lets a user transfer a star', async() => {
    ...
    let user1 = accounts[1];
    let user2 = accounts[2];
    await instance.createStar('Newest Star!', 9, {from: user1});
...
it('lookUptokenIdToStarInfo test', async() => {
    // 1. create a Star with different tokenId
    let instance = await StarNotary.deployed();
    await instance.createStar('Absolute Newest Star!', 10, {from: accounts[0]});
    // 2. Call your method lookUptokenIdToStarInfo
    let starname = await instance.lookUptokenIdToStarInfo(10);
    // 3. Verify if you Star name is the same
    assert.equal("Absolute Newest Star!", starname)
});